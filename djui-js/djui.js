...
@font-face {font-family: FONT_MACHINE; src: url('djui-js/machine.ttf') format('truetype'); font-weight: normal; font-style: normal;}
`;
document.head.appendChild(fontStyles);

context.font = '24px FONT_NORMAL';
currentFont = 'FONT_MACHINE'
currentFontSize = 32;
...
function djui_hud_print_text(text, x, y, scale, maxWidth) {
    ...
    const textWidth = djui_hud_measure_text(text) * scale;
    const hScale = maxWidth != null ? Math.min(maxWidth / textWidth, 1) : 1;

    context.save();
    context.translate(x * resScale, y * resScale);
